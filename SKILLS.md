---
name: minions-retainers
id: OC-0100
version: 1.0.0
description: "Agent skills for working with Minions Retainers MinionTypes. Provides CRUD operations, CLI usage, and best practices for AI agents managing minions-retainers data."
category: ai
subcategory: general
...
# Minions Retainers Agent Skills

Skills for agents operating on the `minions-retainers` toolbox.
...
```bash
# TypeScript
pnpm add @minions-retainers/sdk

# Python
pip install minions-retainers

# CLI
pnpm add -g @minions-retainers/cli
```
...
The `retainers` CLI provides basic project info and utilities:
...
```ts
import { customTypes } from '@minions-retainers/sdk/schemas';
...
```python
from minions_retainers.schemas import custom_types
...
- This agent only writes to `minions-retainers` — it reads from other toolboxes but never writes to them